...
public class TabuSearch {
    ...
    // Represents a neighbor solution and the node which was modified to produce this neighbor.
    ...
    public static List<Node> vertexCoverTabuSearch(List<Node> nodes, List<Edge> edges) {
        ...
        // List of recent moves, used to forbid some moves for a limited number of iterations.
        ...
        while (iterationWithoutImprovement < MAX_ITERATIONS_WITHOUT_IMPROVEMENT) {
            // Find the best neighbor which is not tabu or breaks the tabu but offers a better solution.
            ...
            if (new Random().nextDouble() < 0.02) { // 5% chance
    ...
    // Computes the ratio of weight to the number of edges a node covers. Used in the greedy initialization.
    ...
    // Returns the node with the minimum weight-to-edges-covered ratio. Used in the greedy initialization.
    ...
    // Determines if the new solution has a lower total weight than the current best solution.
    ...
    private static NeighborResult findBestNeighbor(Set<Node> current, List<Node> nodes, List<Edge> edges, Queue<Node> tabuList, Set<Node> bestSolutionEver) {
        ...
        int randomNeighborsToExplore = (int) (nodes.size() * 0.99); // e.g., explore 20% of all nodes
Collections.shuffle(nodes);
List<Node> subsetOfNodes = nodes.subList(0, Math.min(randomNeighborsToExplore, nodes.size()));

        for (Node node : subsetOfNodes) {
            // Toggle the presence of the node in the current solution to produce a neighbor.
    ...
    // Toggles the presence of a node in a set (if present, remove it; if absent, add it).
    ...
    private static boolean isValidNeighbor(Set<Node> neighbor, List<Edge> edges, Node node, Queue<Node> tabuList, Set<Node> bestSolutionEver) {
    ...
    // Adds a node to the tabu list and ensures that the tabu list does not exceed its maximum size.
